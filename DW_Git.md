...
Через эту ссылку помощник попадает в репрезиторий, после этого он может в вкладке "Код" взять ссылку для клонирования шедевра. он создает пустую папку у себя на компе, включает VS Code и вводит в терминал команду: git clone (нужная ссылка без скобочек).
